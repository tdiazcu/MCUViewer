...
bool NgspiceTraceProbe::startTrace(const TraceProbeSettings& probeSettings, uint32_t coreFrequency, uint32_t tracePrescaler, uint32_t activeChannelMask, bool shouldReset)
{
	...
	if (result == 0)
	{
		logger->info("Starting Ngspice reader thread!");
		return true;
	}

	logger->info("Error starting Ngspice reader thread! Error code {}", result);
	return true;
}
...
std::string NgspiceTraceProbe::getTargetName()
{
